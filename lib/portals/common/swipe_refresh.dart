...
class SwipeRefreshLayout extends StatefulWidget {
  const SwipeRefreshLayout({
    ...
    this.onSwipeUp, //load more不添加也就是不需要上拉刷新
    this.backgroundColor,
    this.child,
  })
      : assert(onSwipeDown != null),
        assert(child != null),
        super(key: key);

  //load
  final RefreshCallback onSwipeDown;

  //load more
  final RefreshCallback onSwipeUp;
...
class _SwipeRefreshLayoutState extends State<SwipeRefreshLayout>
  ...
  void _start(bool upper) {
    ...
    current
      ...
      if (mounted && _mode == _RefreshMode.snap) {
        assert(widget.onSwipeDown != null);
        _update(mode);
        Future<void> refresh = upper ? widget.onSwipeDown() : widget.onSwipeUp();
        assert(() {
          if (refresh == null)
  ...
  Future<void> _dismiss(_RefreshMode mode) async {
    _update(mode);
    if (_dragOffset > 0) {
